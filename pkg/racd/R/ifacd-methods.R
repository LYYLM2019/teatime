...
# specification
#------------------------------------------------------------------------------
# Why do we not allow a multispec spec object?
# Because it makes not sense to speak about individual specification
# of ICA factors
.ifacdspec = function(
		mean.model = list(
		...
		distribution.model = list(model = "nig", 
		...
		ica = c("fastica", "radical"), ica.fix = list(A = NULL, K = NULL), ...)
{
	...
	modeldata = list()
	modeldesc = list()
	if(is.null(distribution.model)) distribution.model = "manig"
	...
	if(!any(vmodel$model == valid.model)) 
		stop("\nifacdspec-->error: the garch model does not appear to be a valid choice.\n", call. = FALSE)
	...
	valid.distributions = c("nig", "ghyp")
	if(!any(dmodel$model == valid.distributions)) stop("\nInvalid Distribution model choice\n", call. = FALSE)
	marginal = switch(dmodel$model,
			manig = "nig",
			magh = "ghyp")
	modeldesc$distribution = dmodel$distribution
	...
	if(is.null(ica)){
		...
		if(!any(tolower(ica[1]) == valid.models)) stop("\nifacdspec-->error: Invalid ICA model chosen\n", call. = FALSE)
...
ifacdspec = function(mean.model = list(
				model = c("constant", "AR", "VAR"), robust = FALSE, lag = 1, 
				lag.max = NULL, lag.criterion = c("AIC", "HQ", "SC", "FPE"), 
				external.regressors = NULL, robust.control = list("gamma" = 0.25, 
						"delta" = 0.01, "nc" = 10, "ns" = 500)), 
...
setMethod("ifacdspec",  definition = .ifacdspec)

#------------------------------------------------------------------------------
# estimation (fit)
#------------------------------------------------------------------------------
ifacdfit = function(spec, data, out.sample = 0, solver = "msoptim", 
		fit.control = list(stationarity = 0, scale = 0, n.sim = 2000), 
		solver.control = list(), cluster = NULL, VAR.fit = NULL, 
		ARcoef = NULL, ...)
{
	UseMethod("ifacdfit")
}

setMethod("ifacdfit",  signature(spec = "IFACDspec"), definition = .ifacdfit)


#------------------------------------------------------------------------------
# filter
#------------------------------------------------------------------------------
ifacdfilter = function(fit, data, out.sample = 0, n.old = NULL, 
		cluster = NULL, ...)
{
	UseMethod("ifacdfilter")
}

setMethod("ifacdfilter",  signature(fit = "IFACDfit"), definition = .ifacdfilter)


#------------------------------------------------------------------------------
# forecast
#------------------------------------------------------------------------------
ifacdforecast = function(fit, n.ahead = 10, n.roll = 0, 
		external.forecasts = list(mregfor = NULL), cluster = NULL, ...)
{
	UseMethod("ifacdforecast")
}

setMethod("ifacdforecast",  signature(fit = "IFACDfit"), definition = .ifacdforecast)

#------------------------------------------------------------------------------
# simulation
#------------------------------------------------------------------------------
ifacdsim = function(fit, n.sim = 1, n.start = 0, m.sim = 1, 
...
setMethod("ifacdsim",  signature(fit = "IFACDfit"), definition = .ifacdsim)

#------------------------------------------------------------------------------
# rolling estimation
#------------------------------------------------------------------------------
ifacdroll = function(spec, data, n.ahead = 1, forecast.length = 50, 
...
setMethod("ifacdroll",  signature(spec = "IFACDspec"), definition = .ifacdroll) 
#------------------------------------------------------------------------------
# show
#------------------------------------------------------------------------------
setMethod("show",
		signature(object = "IFACDspec"),
		function(object){
...
setMethod("show",
		signature(object = "IFACDforecast"),
		function(object){
			model = object@model
			cat(paste("\n*------------------------------*", sep = ""))
			cat(paste("\n*        IFACD Forecast        *", sep = ""))
...
setMethod("show",
		signature(object = "IFACDsim"),
		function(object){
			model = object@model
			cat(paste("\n*------------------------------*", sep = ""))
			cat(paste("\n*      IFACD Simulation        *", sep = ""))
			cat(paste("\n*------------------------------*", sep = ""))
			cat(paste("\n\nMean Model\t\t: ", toupper(names(object@model$modelinc[1:3])[object@model$modelinc[1:3]>0]), sep = ""))
			if(sum(object@model$modelinc[2:3])>0){
				cat(paste("\n(Lag)\t\t\t: ", sum(object@model$modelinc[2:3]), sep = ""))
...
# likelihood
#------------------------------------------------------------------------------
.ifacdllh = function(object)
{
	switch(class(object)[1],
			IFACDfit = object@mfit$llh,
			IFACDfilter = object@mfilter$llh)
}

setMethod("likelihood", signature(object = "IFACDfit"), .ifacdllh)
setMethod("likelihood", signature(object = "IFACDfilter"), .ifacdllh)
#------------------------------------------------------------------------------
# sigma
#------------------------------------------------------------------------------
.ifacdfitsig = function(object, factors = TRUE)
{
	T = object@model$modeldata$T
	D = object@model$modeldata$index[1:T]
	ans = switch(class(object)[1],
			IFACDfit = object@mfit$factor.sigmas,
			IFACDfilter = object@mfilter$factor.sigmas)
	if(factors){
		sol = xts(ans[1:T, ], D)
		...
		sol =  try(.Call("ifacdSigma", S = as.matrix(ans), 
						A = as.matrix(A), PACKAGE = "racd"), silent=TRUE)
...
setMethod("sigma", signature(object = "IFACDfit"), .ifacdfitsig)
setMethod("sigma", signature(object = "IFACDfilter"), .ifacdfitsig)

.ifacdforcsig = function(object, factors = TRUE)
{
	T = object@model$modeldata$T
	...
	if(factors){
		...
		for(i in 1:(n.roll+1)){
			ans[,,i] = sqrt(.Call("ifacdSigma", S = matrix(Sx[,,i]^2, ncol = ma), 
					A = A, PACKAGE = "racd"))
		}
...
setMethod("sigma", signature(object = "IFACDforecast"), .ifacdforcsig)

.ifacdsimsig = function(object, sim = 1, factors = TRUE)
{
	T = object@model$modeldata$T
	m = NCOL(object@model$modeldata$data)
	...
	if(factors){
		...
		A = as.matrix(object, which = "A")
		ma = NCOL(A)
		ans = sqrt(.Call("ifacdSigma", S = matrix(ans^2, ncol = ma), 
							A = A, PACKAGE = "racd"))
...
setMethod("sigma", signature(object = "IFACDsim"), .ifacdsimsig)
...
.ifacdsigroll = function(object, factors = TRUE)
{
...
setMethod("sigma", signature(object = "IFACDroll"), .ifacdsigroll)

#------------------------------------------------------------------------------
# coef
#------------------------------------------------------------------------------
.ifacdcoef = function(object)
{
	cf = switch(class(object)[1],
			IFACDfit = object@mfit$garchcoef,
			IFACDfilter = object@mfilter$garchcoef,
			IFACDforecast = object@mforecast$garchcoef,
			IFACDsim = object@msim$garchcoef)
	m = dim(cf)[2]
	colnames(cf) = paste("F_", 1:m, sep = "")
	return(cf)
}

setMethod("coef", signature(object = "IFACDfit"), .ifacdcoef)
setMethod("coef", signature(object = "IFACDfilter"), .ifacdcoef)
setMethod("coef", signature(object = "IFACDforecast"), .ifacdcoef)
setMethod("coef", signature(object = "IFACDsim"), .ifacdcoef)

.ifacdrollccoef = function(object)
{
...
setMethod("coef", signature(object = "IFACDroll"), .ifacdrollccoef)
#------------------------------------------------------------------------------
# fitted
#------------------------------------------------------------------------------
.ifacdfitted = function(object)
{
	ans = switch(class(object)[1],
			IFACDfit = object@mfit$mu,
			IFACDfilter = object@mfilter$mu)
	T = object@model$modeldata$T
	D = object@model$modeldata$index[1:T]
	ans = xts(ans[1:T,], D)
	colnames(ans) = object@model$modeldata$asset.names
...
setMethod("fitted", signature(object = "IFACDfit"), .ifacdfitted)
setMethod("fitted", signature(object = "IFACDfilter"), .ifacdfitted)

.ifacdforcfitted = function(object)
{
	T = object@model$modeldata$T
	m = NCOL(object@model$modeldata$data)
...
setMethod("fitted", signature(object = "IFACDforecast"), .ifacdforcfitted)

.ifacdfitroll = function(object)
{
	rind = object@model$rollind
	index = object@model$index
	s = object@model$out.sample
...
setMethod("fitted", signature(object = "IFACDroll"), .ifacdfitroll)

#------------------------------------------------------------------------------
# residuals
#------------------------------------------------------------------------------
.ifacdresids = function(object)
{
	T = object@model$modeldata$T
	D = object@model$modeldata$index[1:T]
...
setMethod("residuals", signature(object = "IFACDfit"), .ifacdresids)
setMethod("residuals", signature(object = "IFACDfilter"), .ifacdresids)
#------------------------------------------------------------------------------
# ICA matrices
#------------------------------------------------------------------------------
as.matgofit = function(x, rownames.force = NA, which = "A")
{
	...
	if(!any(tolower(which) == tolower(validmat))) stop("Invalid type for ifacd matrix chosen. Valid choices as A, W, U, K, Kinv, E, and D.", call. = FALSE)
	Z = switch(class(x)[1], 
			IFACDfit = x@mfit,
			IFACDfilter = x@mfilter,
			IFACDforecast = x@mforecast,
			IFACDsim = x@msim)
...
setMethod("as.matrix", signature(x = "IFACDfit"), as.matgofit)
setMethod("as.matrix", signature(x = "IFACDfilter"), as.matgofit)
setMethod("as.matrix", signature(x = "IFACDforecast"), as.matgofit)
setMethod("as.matrix", signature(x = "IFACDsim"), as.matgofit)
...
.rcovgarch = function(object, type = 1)
{
	...
	if(type == 1){
		...
		V = try(.Call("ifacdCov", S = sig, A = A, PACKAGE = "racd"), silent=TRUE)
...
setMethod("rcov", signature(object = "IFACDfit"), .rcovgarch)
setMethod("rcov", signature(object = "IFACDfilter"), .rcovgarch)
...
.rcovgarchf = function(object, type = 1)
{
	...
	if(type == 1){
		nam = object@model$modeldata$asset.names
		for(i in 1:(n.roll+1)){
			H[[i]] = try(.Call("ifacdCov", S = matrix(sig[,,i], ncol = m), A = A, PACKAGE = "racd"), silent=TRUE)
...
setMethod("rcov", signature(object = "IFACDforecast"), .rcovgarchf)
...
setMethod("rcov", signature(object = "IFACDsim"), .rcovgarchsim)
...
setMethod("rcov", signature(object = "IFACDroll"), .rcovroll)
...
.rcorgarch = function(object)
{
	...
	D = as.character(object@model$modeldata$index[1:T])
	nam = object@model$modeldata$asset.names
	R = try(.Call("ifacdCor", S = sig, A = A, PACKAGE = "racd"), silent=TRUE)
	dimnames(R) = list(nam, nam, D)
	return( R )
}

setMethod("rcor", signature(object = "IFACDfit"), .rcorgarch)
setMethod("rcor", signature(object = "IFACDfilter"), .rcorgarch)
...
.rcorgarchf = function(object)
{
	...
	nam = object@model$modeldata$asset.names
	for(i in 1:(n.roll+1)){
		R[[i]] = try(.Call("ifacdCor", S = matrix(sig[,,i], ncol = m), A = A, PACKAGE = "racd"), silent=TRUE)
...
setMethod("rcor", signature(object = "IFACDforecast"), .rcorgarchf)
...
setMethod("rcor", signature(object = "IFACDsim"), .rcorgarchsim)
...
setMethod("rcor", signature(object = "IFACDroll"), .rcorroll)
...
.rcoskew = function(object, standardize = TRUE, from = 1, to = 1, roll = 0)
{
	A = as.matrix(object, which = "A")
	m = NROW(A)
	if(is.character(roll) && roll=="all" && class(object) == "IFACDforecast"){
		from = 1
		to = 1
		...
		xs = .Call("ifacdCS", S = yskew, A = A, PACKAGE = "racd")
		if(standardize){
			for(i in 1:n.roll){
		...
		if(standardize){
			for(i in 1:n){
				SD = sqrt(diag(A%*%diag(sig[i,]^2)%*%t(A)))
				xs[,,i] = xs[,,i]/racd:::.coskew.sigma(SD)
			}
		}
...
setMethod("rcoskew", signature(object = "IFACDfit"), .rcoskew)
setMethod("rcoskew", signature(object = "IFACDfilter"), .rcoskew)
setMethod("rcoskew", signature(object = "IFACDforecast"), .rcoskew)
...
setMethod("rcoskew", signature(object = "IFACDroll"), .rcoskewroll)
...
setMethod("rcoskew", signature(object = "IFACDsim"), .rcoskewsim)
...
.rcokurt = function(object, standardize = TRUE, from = 1, to = 1, roll = 0)
{
	...
	if(is.character(roll) && roll=="all" && class(object) == "IFACDforecast"){
		...
		xs = .Call("ifacdCK", K = ykurt, S = sig^2, A = A, PACKAGE="racd")
		if(standardize){
			for(i in 1:n.roll){
				SD = sqrt(diag(A%*%diag(sig[i,]^2)%*%t(A)))
...
setMethod("rcokurt", signature(object = "IFACDfit"), .rcokurt)
setMethod("rcokurt", signature(object = "IFACDfilter"), .rcokurt)
setMethod("rcokurt", signature(object = "IFACDforecast"), .rcokurt)
...
setMethod("rcokurt", signature(object = "IFACDroll"), .rcokurtroll)
...
setMethod("rcokurt", signature(object = "IFACDsim"), .rcokurtsim)
...
setMethod("gportmoments", signature(object = "IFACDfit"), .gportmoments)
setMethod("gportmoments", signature(object = "IFACDfilter"), .gportmoments)
...
setMethod("gportmoments", signature(object = "IFACDforecast"), .gportmomentsf)
...
setMethod("gportmoments", signature(object = "IFACDroll"), .gportmoments.garchroll)
...
setMethod("gportmoments", signature(object = "IFACDsim"), .gportmoments.garchsim)
...
setMethod("nportmoments", signature(object = "IFACDfft"), .nportmoments)
...
.nmoments.fft = function(object, subdivisions = 400, rel.tol = 1e-9, trace = 0, ...)
{
	...
	if(object@model$modtype=="IFACDfit" | object@model$modtype=="IFACDfilter"){
		index = object@model$modeldata$index
		T = object@model$modeldata$T
		nmom = xts(nmom, index[1:T])
	}
	if(object@model$modtype == "IFACDforecast"){
		n.ahead = object@model$n.ahead
		n.roll = object@model$n.roll
...
setMethod("convolution", signature(object = "IFACDfit"), .convolution)
setMethod("convolution", signature(object = "IFACDfilter"), .convolution)
...
setMethod("convolution", signature(object = "IFACDforecast"), .convolutionf)


.convolution.ifacdsim = function(object, weights, fft.step = 0.001, 
		fft.by = 0.0001, fft.support = c(-1, 1), support.method = c("user", "adaptive"), 
...
setMethod("convolution", signature(object = "IFACDsim"), .convolution.ifacdsim)

.convolution.ifacdroll = function(object, weights, fft.step = 0.001, 
		fft.by = 0.0001, fft.support = c(-1, 1), support.method = c("user", "adaptive"), 
		use.ff = TRUE, cluster = NULL, trace = 0, ...)
{
	dist = object@model$modeldesc$distribution
...
setMethod("convolution", signature(object = "IFACDroll"), .convolution.ifacdroll)
	...
	if( support.method == "user" ){
		...
		if( !is.null(cluster) ){
			...
			xpdf = parLapply(cluster, as.list(1:n), fun = function(i){
						racd:::cfinv(z = zz, f = racd:::nigmvcf, 
		...
		if( !is.null(cluster) ){
			...
			xsol = parLapply(cluster, as.list(1:n), fun = function(i){
						...
						ans = racd:::cfinv(z = zz, f = racd:::nigmvcf, 
								step = fft.step, alpha = wpars[,1,i], 
								beta = wpars[,2,i], delta = wpars[,3,i], 
								mu = wpars[,4,i])
	...
	if( support.method == "user" ){
		...
		if( !is.null(cluster) ){
			...
			xpdf = parLapply(cluster, 1:n, fun = function(i){
						racd:::cfinv(z = zz, f = racd:::ghypmvcf, 
			...
			xsol = parLapply(cluster, as.list(1:n), fun = function(i){
						...
						xmax = max(apply(cbind(wpars[,1,i], wpars[,2,i], wpars[,3,i], wpars[,4,i], wpars[,5,i]), 1, 
										FUN = function(x) rugarch:::qgh(1-0.0000001, 
													...
													lambda = x[5])))
						zz = seq(xmin, xmax, by = fft.by)
						ans = racd:::cfinv(z = zz, f = racd:::ghypmvcf, 
								step = fft.step, lambda = wpars[,5,i], 
								alpha = wpars[,1,i], beta = wpars[,2,i], 
								delta = wpars[,3,i], mu = wpars[,4,i])
							sol = list(d = ans, support = c(xmin, xmax))
...
setMethod("dfft", signature(object = "IFACDfft"), .dfft)
...
setMethod("pfft", signature(object = "IFACDfft"), .pfft)
...
setMethod("qfft", signature(object = "IFACDfft"), .qfft)
...
.newsimpact.ifacd = function(object, type = "cov", pair = c(1,2), factor = c(1,2), 
		plot = TRUE, plot.type = c("surface", "contour"))
{
	ans = switch(type,
			cov = .newsimpact.ifacd.cov(object = object, pair = pair, 
					factor = factor, plot = plot, type = plot.type),
			cor = .newsimpact.ifacd.cor(object = object, pair = pair, 
					factor = factor, plot = plot, type = plot.type)
			)
	return(ans)
}

setMethod("nisurface", signature(object = "IFACDfit"), .newsimpact.ifacd)
setMethod("nisurface", signature(object = "IFACDfilter"), .newsimpact.ifacd)

.newsimpact.ifacd.cov = function(object, pair = c(1,2), factor = c(1,2), 
		plot = TRUE, type = c("surface", "contour"))
{
	if(is(object, "IFACDfit")){
...
.newsimpact.ifacd.cor = function(object, pair = c(1,2), factor = c(1,2), 
		plot = TRUE, type = c("surface", "contour"))
{
	if(is(object, "IFACDfit")){
		garchcoef = object@mfit$garchcoef
