...
.ifacdfit = function(spec, data, out.sample = 0, solver = "msoptim", 
		...
		solver.control = list(), cluster = NULL, VAR.fit = NULL, ARcoef = NULL, ...)
{
	tic = Sys.time()
	model = spec@model
	umodel = spec@umodel
	...
	if(!is.numeric(out.sample)) 
		stop("\nifacdfit-->error: out.sample must be numeric\n")
	if(as.numeric(out.sample) < 0) 
		stop("\nifacdfit-->error: out.sample must be positive\n")
	n.start = round(out.sample, 0)
	n = dim(xdata$data)[1]
	if( (n-n.start) < 100) 
		stop("\nifacdfit-->error: function requires at least 100 data\n points to run\n")
	data   = xdata$data
	index  = xdata$index
	period = xdata$period
	...
	#-----------------------------------------------------------------------------------
	# Factor Dynamics
	xspec = acdspec(mean.model = list(include.mean = FALSE, armaOrder=c(0,0)),
			variance.model = list(model = umodel$vmodel, garchOrder = umodel$garchOrder, 
					variance.targeting = umodel$variance.targeting), 
			distribution.model = umodel$dmodel)
	...
	if(any(converge == 1)){
		...
		stop("\nifacdfit-->warning: convergence problem in univariate fit...", call. = FALSE)
	}
	#-----------------------------------------------------------------------------------
...
.ifacdfilter = function(fit, data, out.sample, n.old, cluster = NULL, ...)
{
	tic = Sys.time()
	#-----------------------------------------------------------------------------------
	# Data Extraction
...
.ifacdforecast = function(fit, n.ahead, n.roll, external.forecasts, cluster = NULL, ...)
{
	tic = Sys.time()
	...
	ans = new("IFACDforecast",
			mforecast = tmp,
			model = model)
	return(ans)
}

.ifacdsim = function(fit, n.sim, n.start, m.sim, startMethod, prereturns, 
		preresiduals, presigma, mexsimdata, rseed, cluster = NULL, ...)
{
	...
	if( !is.null(rseed) ){
		if( !is.matrix(rseed) ){
			...
			if( length(rseed) != 1 ) 
				stop("\nifacdsim-->error: if not matrix, rseed must be a single value!\n")
			xseed = matrix(rseed+seq(1,m.sim*m,by=1), ncol = m, nrow = m.sim, byrow=TRUE)
		} else{
			if(dim(rseed)[2] != m) 
				stop("\nifacdsim-->error: rseed must have N (= n.assets) columns!\n")
			if(dim(rseed)[1] != m.sim) 
				stop("\nifacdsim-->error: rseed must have m.sim rows!\n")
			xseed = rseed
		}
	} else{
	...
	if(startMethod == "sample"){
		if(!is.na(presigma[1])){
			...
			if(dim(presigma)[1]<mx) 
				stop(paste("\nifacdsim-->error: presigma must have row dimension ", mx, sep = ""))
	...
	if(startMethod == "sample"){
		if(!is.na(preresiduals[1])){
			...
			if(dim(preresiduals)[1]<mx) 
				stop(paste("\nifacdsim-->error: preresiduals must have row dimension ", mx, sep = ""))
...
#-----------------------------------------------------------------------------------------------
# Rolling
.rollifacd = function(spec, data, n.ahead = 1, forecast.length = 50, 
